...
# library(tabplot)
# tableplot(moddf[,c(1:14, ncol(moddf))], sortCol="CDEC_ID")
# tableplot(moddf[,c(1:2, 15:ncol(moddf))], sortCol="CDEC_ID") 
```
...
# flow_ordered <- sort(moddf$FLOW)
# n <- sum(!is.na(moddf$FLOW))
# # notice the option type = 's' for plotting the step functions
# plot(flow_ordered, (1:n)/n, type = 's', ylim = c(0, 1), xlab = 'Sorted Monthly Unimpaired Flow', ylab = '', main = '', xlim=c(0.1,max(moddf$FLOW)), log='x')
...
png('Output Data/rplot23_boxplot.png', width=6.5, height=3, units="in", pointsize=8, res=300)
...
png('Output Data/rplot24_flowvslat.png', width=6.5, height=3, units="in", pointsize=8, res=300)
...
```{r correlation_plots}
png('Output Data/rplot25_tmpvsppt.png', width=6.5, height=4, units="in", pointsize=8, res=300)
par(mar=c(4,4,1,1)+0.1)
ggplot(moddf, aes(x=PPT, y=TMP, size=FLOW, fill=CDEC_ID)) +
  ...
  theme_bw() +
  scale_fill_manual(values = c(cbpblack, cbpgrey[1])) +
  guides(fill = guide_legend(override.aes = list(alpha = 1)))
dev.off()

png('Output Data/rplot26_pptvsflow.png', width=6.5, height=4, units="in", pointsize=10, res=300)
par(mar=c(4,4,1,1)+0.1)
...
png('Output Data/rplot27_pptvsarea.png', width=6.5, height=4, units="in", pointsize=10, res=300)
par(mar=c(4,4,1,1)+0.1)
