...
nnloss <- function(data, losstype){
  ...
  for (k in 1:(length(unique(data$CDEC_ID)))){
    ...
    trainsetpvs2 <- as.matrix(trainset[,c(10, 13:36, 38)]) 
    ...
    # This is not working inside the function environment, because trainsetpvs2 is only saved inside the this function and not in global, so the wrapper is not able to locate it. So, let's take the easy way out and just take the asymmetric part out of the function and run the code chunk below instead.
    if(losstype=="wlse"){
      nnmodel %>% 
...
```{r nn_wlse}
# # function environments not saving trainsetpvs2 for some reason. Let's try outside of the nnloss function
# optimmodel <- results <- modgof <- list()
...
#   trainset <- merge(trainset, floodlvl_mean, by="CDEC_ID")
#   trainset$FLOOD <- ifelse(trainset$PPT>trainset$FLVL_MEAN, 1, 0)
#   trainsetpvs2 <- as.matrix(trainset[,c(10, 13:36, 39)])
#   trainsetrv2 <- as.matrix(trainset$FLOW)
# 
#   # normalize the training set on standard deviation, then normalize testset by using mean and SD of training set so no leaks happen
#   scale_par <- preProcess(trainsetpvs, method=c("scale"))
...
#   # for asymmetric losses: designate each observation as a flood:1 or a drought:0 for asymmetric losses
#   floodlvl_mean <- aggregate(PPT~CDEC_ID, data=trainset, FUN=mean)
#   # floodlvl_median <- aggregate(PPT~CDEC_ID, data=trainset, FUN=median) # let's try median
#   colnames(floodlvl_mean)[2] <- "FLVL_MEAN"
#   trainset <- merge(trainset, floodlvl_mean, by="CDEC_ID")
#   trainset$FLOOD <- ifelse(trainset$PPT>trainset$FLVL_MEAN, 1, 0)
#   trainsetpvs2 <- as.matrix(trainset[,c(10, 13:36, 39)])
...
mean(unlist(nn_logo_mse_results[["gof"]]["bR2", ]))
mean(unlist(nn_logo_mae_results[["gof"]]["bR2", ]))
mean(unlist(nn_logo_msle_results[["gof"]]["bR2", ]))
mean(unlist(nn_logo_mape_results[["gof"]]["bR2", ]), na.rm=TRUE)
mean(unlist(nn_logo_logcosh_results[["gof"]]["bR2", ]))
mean(unlist(nn_logo_mspe_results[["gof"]]["bR2", ]), na.rm=TRUE)
mean(unlist(nn_logo_male_results[["gof"]]["bR2", ]))
mean(unlist(nn_logo_msale_results[["gof"]]["bR2", ]))
mean(unlist(nn_logo_wlse_results[["gof"]]["bR2", ]))
mean(unlist(nn_logo_linexe_results[["gof"]]["bR2", ]))
```
...
for (r in 1:(length(unique(results$CDEC_ID)))){ 
  ...
  png(paste0('Output Data/ts_loss/timeseries_nn_', h, '.png'), width=6.5, height=8, units="in", pointsize=8, res=1200)
    print(hydrograph)
  dev.off()
}
```
...
for (r in 1:(length(unique(results$CDEC_ID)))){ 
  ...
  png(paste0('Output Data/ts_loss_zoomed/timeseries_nn_', h, '.png'), width=6.5, height=8, units="in", pointsize=8, res=1200)
    print(hydrograph)
  dev.off()
}
```
...
gofdotchart <- function(gof_results_mse, gof_results_logcosh, gof_results_mae, gof_results_mspe, gof_results_wlse, gof_results_linexe, modeltype, fitofinterest){
  ...
  png(paste0('Output Data/rplot310_', modeltype, "_", fitofinterest, 'dotchart_comp.png'), width=6.5, height=8, units="in", pointsize=8, res=1200)
    print(plottoprint)
  dev.off()
}
