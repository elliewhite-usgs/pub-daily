...
# groupingstyle can be: resub, 2L, 5L, 10L, logo, lmgo. Notice how a plain 2, 5, and 10 will not work, you have to declare it an integer
...
glmcv <- function(data, groupingstyle, datatype){  
  ...
  # # for leave hierarchies out cross validation, not trying this
  # if(groupingstyle=="lho"){
  # }
...
# we are not saving the models here because it takes too much memory. so rfmodel is no longer specified as a list and took mod=rfmodel out of return list. 
...
Use a rolling window to connect observations. 
...
Here the input shape is different. Where the model sees data at a specific time for all the basins. So, here, hydrology is defined as a process that happens at 19 basins at once. Asking the model to predict one basin will no longer make sense. So, !!! NOT for PUB where the ungauged basin is in space. For PUB when there's a timeseries or a forecast that is of interest. 
...
## 6.7 Variable Importance Plots 
```{r vip_prep}
# the models need to be saved, so run the modeling again, these plots from here on out take a long time to process too! 
...
## 6.9 Next Steps Plots
```{r LSTM_plot}
# remember results gets truncated by ncells for each basin
ncells <- 3
lstm_logo_results <- readRDS("Output Data/rds/lstm_logo_exp8_results.RDS")
results <- pp_results(lstm_logo_results, results, "lstm", "logo")

mastercv_lstm <- results[,c("CDEC_ID", "FLOW", "LSTM_LOGOFIT")]
mastercv_lstm$MODELTYPE <- "LSTM"
colnames(mastercv_lstm) <- c("CDEC_ID", "OBS", "LOGO", "MODELTYPE")
  
# just do logo nn and lstm
master_cv <- rbind(mastercv_nn[, c("CDEC_ID", "OBS", "LOGO", "MODELTYPE")], mastercv_lstm)
master_cv_melted <- melt(master_cv, measure.vars= c("LOGO"), variable.name="GROUPINGSTYLE", value.name="PRED")
master_cv_melted$MODELTYPE <- factor(master_cv_melted$MODELTYPE, levels=c("NN", "LSTM"))
master_cv_melted$GROUPINGSTYLE <- factor(master_cv_melted$GROUPINGSTYLE, levels=c("LOGO"))
pretty_facet_labels <- c("Leave One Group Out (LOGO)")
names(pretty_facet_labels) <- c("LOGO")

png('Output Data/rplot43_obsvspred_logonnlstm.png', width=6.5, height=6.5/2, units="in", pointsize=8, res=300)
  plottoprint <- ggplot(master_cv_melted, aes(x=OBS, y=PRED)) +
    geom_point(aes(group=CDEC_ID, colour=CDEC_ID, shape=CDEC_ID)) +
    geom_smooth(method="lm", se=FALSE, color="black")+
    stat_poly_eq(formula = y ~ x,
